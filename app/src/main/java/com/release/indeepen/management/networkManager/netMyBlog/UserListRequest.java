 ...
 */
public class UserListRequest extends NetworkRequest<WriterResultList> {
    String sURL;
    ...
    @Override
    public WriterResultList parsing(InputStream is) {
        WriterResultList result = null;

        try {
            JsonReader reader = new JsonReader(new InputStreamReader(is, "UTF-8"));

            result = new Gson().fromJson(reader, WriterResultList.class);
            is.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
