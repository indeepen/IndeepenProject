 ...
 */
public class MyBlogProfileRequest extends NetworkRequest<UserProfileResult> {
    String sURL;
    ...
    @Override
    public UserProfileResult parsing(InputStream is) {
        UserProfileResult result = null;

        try {
            JsonReader reader = new JsonReader(new InputStreamReader(is, "UTF-8"));
            result = new Gson().fromJson(reader, UserProfileResult.class);
            is.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
