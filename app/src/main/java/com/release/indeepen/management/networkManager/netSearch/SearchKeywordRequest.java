 ...
 */
public class SearchKeywordRequest extends NetworkRequest<SearchAllResult> {
    String sURL;
    ...
    @Override
    public SearchAllResult parsing(InputStream is) {
        SearchAllResult result = null;

        try {
            JsonReader reader = new JsonReader(new InputStreamReader(is, "UTF-8"));

            result = new Gson().fromJson(reader, SearchAllResult.class);
            is.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
